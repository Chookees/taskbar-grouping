...
**Asset size baseline (v0.4.6, measured from the published release):** portable ZIP ≈ 167 MB (166,516,832 B), Setup.exe ≈ 112 MB (112,094,558 B) — a few KB under v0.4.5, as a five-line XAML change should be. A collapse to a fraction of this means a publish step produced a framework-dependent build. `PublishReadyToRun=true` (Launcher + Manager) roughly doubled the size vs v0.2.1 because the entire .NET runtime is AOT-compiled into the publish. Plan estimates that assume "~10 MB R2R cost" are wildly low — count on +50-70 MB per binary.
